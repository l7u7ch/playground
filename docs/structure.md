...
│   │   ├── actions.ts                 — Server Actions: addTodo（フォームデータから title と deadline を todos テーブルへ挿入）
│   │   └── page.tsx                   — ホームページ（Server Component）: todos を createdAt 順に全件取得して TodosTable へ渡す
│   ├── components/
│   │   ├── AddTodoModal.tsx           — ToDo 追加モーダル（HeroUI Modal + Form）の Client Component: title と deadline を入力
│   │   └── TodosTable.tsx             — TanStack Table を使用した ToDo 一覧の Client Component: deadline カラムの表示・検索・ソート・ページネーション対応
│   ├── lib/
│   │   └── db.ts                      — Drizzle ORM クライアントファクトリ: DATABASE_URL から postgres 接続を生成して返す
│   └── schema.ts                      — Drizzle ORM スキーマ: todos テーブル（id, title, completed, createdAt, deadline）を定義
